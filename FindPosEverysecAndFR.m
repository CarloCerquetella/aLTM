function FindPosEverysecAndFR(nameSess,sessionLTMI,nCells,tet2useI,cellsI)
...
GroupCells=nan(800,(CellsNum+4)*12);
IndexCell=1;
...
for tzu= 1:size(ArrayTet,2)
    ...
    for ti= 1:size(ArrayCells,2)
        ...
        IndexConstAcrossSess=1:(CellsNum+4):size(GroupCells,2);
        if IndexCell == 1
            GroupCells(1:800,IndexConstAcrossSess)=Seconds;
            GroupCells(1:800,IndexConstAcrossSess+1)=PosOfAnimalPerSec;
            GroupCells(1:800,IndexConstAcrossSess+2)=PosOfAnimalPerSecInBin;
            GroupCells(1:800,IndexConstAcrossSess+3)=TypeConflArray;
        end
        GroupCells(1:800,IndexConstAcrossSess+3+IndexCell)=ZscoredMatrix;
...
% for indexSess=1:12
%     if indexSess<12
%         tabletoUse=GroupCells(1:800,IndexConstAcrossSess(1,indexSess):IndexConstAcrossSess(1,indexSess+1)-1);
%     else
%         tabletoUse=GroupCells(1:800,IndexConstAcrossSess(1,indexSess):size(GroupCells,2));
%     end
...
nametable=strcat('Time-Position-Bin-ZScoredFRPerCell_AllSess.xlsx');
if isfile(nametable)==1
    delete(nametable);
end
writetable(arraytable,nametable);

cd('T:\Carlo\FilesForCamille')
nametable=strcat(nameSess,'Time-Position-Bin-ZScoredFRPerCell_AllSess.xlsx');
if isfile(nametable)==1
    delete(nametable);
end
writetable(arraytable,nametable);

...
# Data for training
recordings_labels_df_1 = recordings_labels_df[recordings_labels_df["Labels"].isin(["1","5"])]
recordings_labels_df_1 = recordings_labels_df_1.reset_index(drop=True)
dataset_1 = CustomDataset(recordings_labels_df_1['Recordings'], recordings_labels_df_1['Labels'])    

n_samples = len(dataset_1)
n_samples_train = int(split_train_test*n_samples)
n_samples_test = n_samples - n_samples_train

train_dataset, val_dataset = torch.utils.data.random_split(dataset_1, [n_samples_train, n_samples_test],
    generator=torch.Generator().manual_seed(42))   

# Data for testing
recordings_labels_df_2 = recordings_labels_df[recordings_labels_df["Labels"].isin(["2","3","4"])]

recordings_labels_df_2 = recordings_labels_df_2.reset_index(drop=True)


test_dataset = CustomDataset(recordings_labels_df_2['Recordings'], recordings_labels_df_2['Labels'])    
...
with torch.no_grad():
    ...
    for data in val_loader:
        ...
        y_pred_val.extend(predicted.cpu().data.numpy()) # Save Prediction
        
        y_true_val.extend(target) # Save Truth
    ...
    for data in test_loader:
        ...
        _, predicted = torch.max(outputs.data, 1)
        y_pred_test.extend(predicted.cpu().data.numpy()) # Save Prediction
        
        y_true_test.extend(target) # Save Truth

###################################################################
        
name = 'results_training_NA_HA_testing_' + os.environ['SLURM_ARRAY_TASK_ID']
res = open(name, 'wb')
pickle.dump([train_accuracy_values,val_accuracy_values,y_pred_val,y_true_val,y_pred_test,y_true_test], res)

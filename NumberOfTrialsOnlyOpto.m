function NumberOfTrialsOnlyOpto(nameSess,sessionLTMI)
...
for isess=1:size(sessionLTMI,2)
    ...
    Open=TableFile{:,12}==1;
    Light=TableFile{:,11}==1;
    Center=TableFile{:,10}==1;
    Closed=TableFile{:,9}==1;
    AllNumb=1:(size(TableFile,1)-1);
    IDBin=find((Center(AllNumb,1)==1 | (Closed(AllNumb,1)==1) | (Light(AllNumb,1)==1)) & Open(AllNumb+1,1)==1);
